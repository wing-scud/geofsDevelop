...
import {fixAngle360 ,fixAngle, clamp,exponentialSmoothing ,V3,PID}from "../utils/utils"
...
controls.init = function() {
    ...
    controls.mouseHandler = function(a) {
        !1 === controls.mouse.down || geofs.api.nativeMouseHandling ? controls.mode == 'mouse' && (controls.mouse.xValue = clamp((a.pageX - controls.mouse.oX - controls.mouse.cX) * controls.mouse.rX, -1, 1),
            controls.mouse.yValue = clamp((a.pageY - controls.mouse.tY - controls.mouse.cY) * controls.mouse.rY, -1, 1),
            controls.keyboard.override = !1) : (controls.mouse.down == 1 && camera.rotate((controls.mouse.originalX - a.pageX) * controls.mouse.orbit.ratioX, (controls.mouse.originalY - a.pageY) * controls.mouse.orbit.ratioY) && (a.preventDefault(),
                a.stopPropagation(),
                a.stopImmediatePropagation()),
            controls.mouse.down == 3 && camera.translate(-(controls.mouse.lastY - a.pageY) * controls.mouse.orbit.ratioZ) && (a.preventDefault(),
                a.stopImmediatePropagation()),
            controls.mouse.lastY = a.pageY);
        geofs.api.nativeMouseHandling || (a.preventDefault(),
            a.stopImmediatePropagation());
    };
    $(document).on('click', '.geofs-orientationReset, .geofs-orientationCalibrate', () => {
        controls.orientation.recenter();
    });
    $('.geofs-canvas-mouse-overlay, .geofs-ui-right').on('mousemove', (a) => {
...
controls.initViewportDimensions = function() {
    let a = $('.geofs-canvas-mouse-overlay'),
        b = a[0];
...
controls.setMode = function(a) {
    ...
    geofs.isMobile && a != 'orientation' && a != 'touch' && (a = 'orientation');
    controls.mode = a;
    controls.mode == 'orientation' ? (controls.orientation.init(),
    ...
    controls.mode == 'touch' ? (controls.exponential = geofs.preferences.touch.exponential,
    ...
    controls.mode == 'joystick' && (controls.exponential = geofs.preferences.joystick.exponential,
    ...
    controls.mode == 'mouse' && (controls.exponential = geofs.preferences.mouse.exponential,
    ...
    controls.mode == 'keyboard' && (controls.exponential = 0,
...
controls.setters = {
    ...
    setOptionalAnimatedPart: {
        ...
        set() {
            controls.optionalAnimatedPart.target = controls.optionalAnimatedPart.target == 0 ? 1 : 0;
    ...
    setGear: {
        ...
        set() {
            if (!geofs.aircraft.instance.groundContact || geofs.debug.on) {
                controls.gear.target = controls.gear.target == 0 ? 1 : 0,
...
controls.update = function(a) {
    ...
    controls.mode == 'joystick' && controls.updateJoystick(a);
    if (controls.autopilot.on) { controls.autopilot.update(a); } else {
        ...
        controls.mode != 'mouse' && controls.mode != 'touch' || controls.keyboard.override || controls.updateMouse(a);
        controls.mode == 'orientation' && controls.updateOrientation(a);
        controls.mode == 'touch' && controls.updateTouch(a);
...
controls.updateKeyboard = function(a) {
    let b = controls.keyboard.rollIncrement * a * geofs.preferences.keyboard.sensitivity;
...
controls.joystick.poll = function() {
    ...
    const a = controls.joystick.api.call(navigator);
    if (a.length > 0) {
        for (let b = 0; a[b] && b < 5;) {
        ...
        controls.joystick.sticksNumber != controls.joystick.sticks.length && (controls.joystick.info = '',
            controls.joystick.sticks.forEach((a) => {
...
controls.joystick.init = function() {
    controls.joystick.api = (typeof navigator.getGamepads === 'function' ? navigator.getGamepads : null) || (typeof navigator.webkitGetGamepads === 'function' ? navigator.webkitGetGamepads : null) || null;
...
controls.joystick.configure = function() {
    ...
    let a = 0,
        b = 0;
    controls.joystick.sticks.forEach((c, d) => {
        c.hash = geofs.utils.hashCode(c.id);
        for (const e in c.buttons) {
            var f = c.hash + e;
            if (geofs.preferences.joystick.buttons[f]) {
                let g = controls.setters[geofs.preferences.joystick.buttons[f]].set,
...
controls.updateJoystick = function(a) {
    if (controls.joystick.poll()) {
        for (var b in controls.joystick.axes) {
            ...
            if (c != 'none' && (c = controls.axisSetters[c])) {
        ...
        if (controls.joystick.buttons) {
            for (b in controls.joystick.buttons) {
                if ((a = controls.joystick.buttons[b]) && geofs.preferences.joystick.buttons[b] != 'none' && (d = a.oldValue,
...
controls.orientation.fixPitch = function(a) {
    a && Math.abs(a[1]) > 90 && (a[0] < 0 && (a[0] += 180),
        a[0] > 0 && (a[0] -= 180));
};
...
controls.updateOrientation = function(a) {
    if (controls.orientation.centers) {
        ...
        const b = controls.orientation.getNormalizedAxis(geofs.preferences.orientation.axis.roll);
...
controls.autopilot.initUI = function() {
    ...
    controls.autopilot.setAltitude = function(a) {
        const b = controls.autopilot.altitude;
        try {
    ...
    controls.autopilot.setKias = function(a) {
        const b = controls.autopilot.kias;
        try {
    ...
    controls.autopilot.setClimbrate = function(a) {
        const b = controls.autopilot.climbrate;
        try {
    ...
    $(document).on('pointerdown touchstart', '.numberUp, .numberDown', function(a) {
        ...
        clearTimeout(window.spinnerRepeat);
    }).on('click', '.geofs-autopilot-pad', (a) => {
        controls.autopilot.toggle();
    });
};
...
controls.autopilot.turnOn = function() {
    if (geofs.aircraft.instance.setup.autopilot) {
        const a = geofs.animation.values;
...
controls.autopilot.update = function(a) {
    ...
    c.rollPID.set(e);
    controls.roll = exponentialSmoothing('apRoll', -c.rollPID.compute(b.aroll, a) / d, 0.9);
    let f = c.altitude - b.altitude;
    e = clamp(d * c.commonClimbrate, 0, c.maxClimbrate);
    const g = clamp(d * c.commonDescentrate, c.maxDescentrate, 0);
    f = clamp(5 * f, g, e);
    c.climbPID.set(-f);
