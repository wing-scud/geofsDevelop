...
import { V3,AIR_PRESSURE_SL,AIR_TEMP_SL,LONGITUDE_TO_HOURS,KELVIN_OFFSET,
    ...
    ,MS_TO_KNOTS} from '../utils/utils'
import geofs from "../geofs"

window.weather = window.weather || {};
weather.dataProxy =`http://localhost:3030/proxy/backend/weather/metar.php?icao=` //`${geofs.url}/backend/weather/metar.php?icao=`;
...
weather.init = function() { //天气初始化，风速，风向
...
weather.generateFromPreferences = function() {//season
...
weather.setDateAndTime = function(a) {
    ...
    geofs.isNight = weather.timeRatio > 0.4;//判断白天黑夜，改变
    geofs.animation.values.night != geofs.isNight && $('body').trigger('nightChange'); //白天黑夜
